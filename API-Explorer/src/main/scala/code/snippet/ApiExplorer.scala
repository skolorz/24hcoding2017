...
class ApiExplorer extends Loggable {
  ...
  def showResources = {
    ...
    val supportedApiVersions = List ("2.1.0")
    ...
    val (catalogHeadline, catalogDescription) = List(showCore, showOBWG, showPSD2)  match {
      ...
      case List(None, None, None) => ("OBP",
        "The full set of Open Bank Project APIs supports functionality including transaction history, payments, customer, counterparty and transaction metadata, delegated account access and data redaction.")
