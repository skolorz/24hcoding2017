...
class TransactionRequestsTest extends ServerSetupWithTestData with DefaultUsers with V140ServerSetup {
  ...
  feature("we can make transaction requests") {
    ...
    if (!Props.getBool("transactionRequests_enabled", false) || Props.getBool("hackathon_apis", false)) {
      ignore("we create a transaction request with a challenge (disabled or sms token needed)", TransactionRequest) {}
    } else {
      scenario("we create a transaction request with a challenge", TransactionRequest) {
        ...
        // TEST IS NOT POSSIBLE, WE USE EXTERNAL APP TO SIMULATE RECEIVING CHALLENGE SMS
        //call answerTransactionRequestChallenge again, give a good answer
        answerJson = ChallengeAnswerJSON(token = "1234") //wrong answer, not a number
        request = (v1_4Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
          "owner" / "transaction-request-types" / "SANDBOX_TAN" / "transaction-requests" / transId / "challenge").POST <@ (user1)
        response = makePostRequest(request, write(answerJson))
