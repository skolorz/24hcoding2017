...
class OAuthTest extends ServerSetup {
  ...
  feature("Verifier"){
    // LOGIN TOKEN NEEDED BEFORE VERIFIER
    if (Props.getBool("hackathon_apis", false)) {
      ignore("the user login and get redirected to the application back (login token needed)", Verifier, Oauth) {}
    } else {
    ...
    // LOGIN TOKEN NEEDED BEFORE VERIFIER
    if (Props.getBool("hackathon_apis", false)) {
      ignore("the user login and is asked to enter the verifier manually (login token needed)", Verifier, Oauth) {}
    } else {
  ...
  feature("access token"){
    // LOGIN TOKEN NEEDED BEFORE VERIFIER
    if (Props.getBool("hackathon_apis", false)) {
      ignore("we get an access token without a callback (login token needed)", AccessToken, Oauth) {}
    } else {
    ...
    // LOGIN TOKEN NEEDED BEFORE VERIFIER
    if (Props.getBool("hackathon_apis", false)) {
      ignore("we get an access token with a callback (login token needed)", AccessToken, Oauth) {}
    } else {
    ...
    // LOGIN TOKEN NEEDED BEFORE VERIFIER
    if (Props.getBool("hackathon_apis", false)) {
      ignore("we don't get an access token because the request token is wrong (login token needed)", AccessToken, Oauth) {}
    } else {
