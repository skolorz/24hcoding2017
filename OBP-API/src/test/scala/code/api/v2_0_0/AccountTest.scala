...
class AccountTest extends V200ServerSetup with DefaultUsers {
  ...
  feature("Assuring that Get all accounts at all banks works as expected - v2.0.0") {
    ...
    if (Props.getBool("hackathon_apis", false)) {
      ignore("We create an account and get accounts as anonymous and then as authenticated user - allAccountsAllBanks (stripped for hackathon)") {}
    } else {
      scenario("We create an account and get accounts as anonymous and then as authenticated user - allAccountsAllBanks") {
        ...
        val requestPut = (v2_1Request / "banks" / testBank.value / "accounts" / newAccountId1).PUT <@ (user1)
        ...
        val requestGet = (v2_1Request / "accounts").GET
        ...
        val requestGetAll = (v2_1Request / "accounts").GET <@ (user1)
    ...
    if (Props.getBool("hackathon_apis", false)) {
      ignore("We create an account and get accounts as anonymous and then as authenticated user - allAccountsAtOneBank (stripped for hackathon)") {}
    } else {
      scenario("We create an account and get accounts as anonymous and then as authenticated user - allAccountsAtOneBank") {
        ...
        When("We make the anonymous access request")
        val requestGet = (v2_1Request / "banks" / testBank.value / "accounts").GET
        val responseGet = makeGetRequest(requestGet)
        ...
        When("We make the authenticated access request")
        val requestGetAll = (v2_1Request / "banks" / testBank.value / "accounts").GET <@ (user1)
        val responseGetAll = makeGetRequest(requestGetAll)
