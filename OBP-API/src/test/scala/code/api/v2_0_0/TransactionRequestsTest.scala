...
class TransactionRequestsTest extends ServerSetupWithTestData with DefaultUsers with V200ServerSetup {
  ...
  feature("we can make transaction requests") {
    ...
    if (!Props.getBool("transactionRequests_enabled", false) || Props.getBool("hackathon_apis", false)) {
      ignore("we create a transaction request with a user who doesn't have access to owner view but has CanCreateAnyTransactionRequest at BANK_ID  (stripped for hackathon)", TransactionRequest) {}
    } else {
      scenario("we create a transaction request with a user who doesn't have access to owner view but has CanCreateAnyTransactionRequest at BANK_ID", TransactionRequest) {
        ...
        val toAccountJson = TransactionRequestAccountJSON(toAccount.bankId.value, toAccount.accountId.value, toAccount.iban.getOrElse(""))
    ...
    // No challenge, No FX (same currencies)
    if (!Props.getBool("transactionRequests_enabled", false)  || Props.getBool("hackathon_apis", false)) {
      ignore("we create a transaction request without challenge, no FX (same currencies) (stripped for hackathon)", TransactionRequest) {}
    } else {
    ...
    if (!Props.getBool("transactionRequests_enabled", false) || Props.getBool("hackathon_apis", false)) {
      ignore("we create a transaction request with a challenge, same currencies (disabled or stripped for hackathon)", TransactionRequest) {}
    } else {
      scenario("we create a transaction request with a challenge, same currencies", TransactionRequest) {
        ...
        var answerJson = ChallengeAnswerJSON(token = "hello") //wrong answer, not a number
        request = (v1_4Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
        ...
        answerJson = ChallengeAnswerJSON(token = "1234") //good answer, not a number
        request = (v1_4Request / "banks" / testBank.bankId.value / "accounts" / fromAccount.accountId.value /
          "owner" / "transaction-request-types" / "SANDBOX_TAN" / "transaction-requests" / transRequestId / "challenge").POST <@ (user1)
        response = makePostRequest(request, write(answerJson))
    ...
    if (!Props.getBool("transactionRequests_enabled", false) || Props.getBool("hackathon_apis", false)) {
      ignore("we create an FX transaction request with challenge (disabled or stripped for hackathon)", TransactionRequest) {}
    } else {
