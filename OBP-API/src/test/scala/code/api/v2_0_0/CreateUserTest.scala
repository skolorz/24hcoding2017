...
class CreateUserTest extends V200ServerSetup with BeforeAndAfter {
  ...
  feature("we can create an user and login as newly created user using both directLogin and OAuth") {
    ...
    if (Props.getBool("hackathon_apis", false)) {
      ignore("we create an user with email, first name, last name , username and password (stripped for hackathon)", CreateUser) {}
    } else {
      scenario("we create an user with email, first name, last name , username and password", CreateUser) {
        ...
        var request = (v2_1Request / "users").POST
    ...
    if (Props.getBool("hackathon_apis", false)) {
      ignore("we login using directLogin as newly created user (stripped for hackathon)", CreateUser) {}
    } else {
    ...
    if (Props.getBool("hackathon_apis", false)) {
      ignore("we try to create a same user again (stripped for hackathon)", CreateUser) {}
    } else {
      scenario("we try to create a same user again", CreateUser) {
        ...
        val request = (v2_1Request / "users").POST
