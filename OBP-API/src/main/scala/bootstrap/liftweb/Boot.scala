...
class Boot extends Loggable{
  ...
  //  if (!disabledVersions.contains("v2_2_0")) LiftRules.statelessDispatch.append(v2_2_0.OBPAPI2_2_0)
    ...
    val entitlementsView = {
      if(Props.getBool("enable_entitlements_view",false)) {
        List(Menu.i("Your entitlements") / "entitlements" >> OBPUser.loginFirst)
      } else {
        Nil
      }
    }
    ...
    val sitemap = List(
          ...
          Menu.i("Your API keys") / "keys" >> OBPUser.loginFirst,
    ...
    ) ++ entitlementsView ++ accountCreation ++ Admin.menus
