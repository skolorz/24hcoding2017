...
import code.api.util.ApiRole.{CanCreateAnyTransactionRequest, CanCreateSandbox}
...
import code.api.v2_0_0.{JSONFactory200, TransactionRequestBodyFromJSON, TransactionRequestBodyJSON}
import code.api.v2_1_0.JSONFactory210.{createConsumerJSONs, _}
...
import org.iban4j.{IbanFormat, IbanUtil}
...
trait APIMethods210 {
  ...
  val Implementations2_1_0 = new Object() {
    ...
    resourceDocs += ResourceDoc(
      ...
      s"""Initiate a Payment via a Transaction Request.
        ...
        |${authenticationRequiredMessage(true)}
         |
        |""",
      Extraction.decompose(TransactionRequestBodyFromJSON (
        TransactionRequestAccountFromJSON("BANK_ID", "ACCOUNT_ID"),
              ...
              createdTransactionRequest <- transactionRequestType.value match {
                case "SANDBOX_TAN" => {
                  for {
                    ...
                    createdTransactionRequest <- Connector.connector.vend.createTransactionRequestv210(u, fromAccount, toAccount, transactionRequestType, transDetails, transDetailsSerialized)
                ...
                case "SEPA" => {
                  for {
                    ...
                    ibanOk <- tryo {IbanUtil.validate(toIban)} ?~! {ErrorMessages.IbanNotValid}
                    foundAccountBox = Connector.connector.vend.getBankAccountByIban(toIban.filterNot((x: Char) => x.isWhitespace))
                  } yield {
                    val foundAccount = if(foundAccountBox.isEmpty) new MappedBankAccount() else  foundAccountBox.get
                    foundAccount.setIban(toIban)
                    Connector.connector.vend.createTransactionRequestv210(u, fromAccount, foundAccount,
                      transactionRequestType, transDetails, transDetailsSerialized).get
                  }
                }
              }
            } yield {
    ...
    val lowAmount  = AmountOfMoneyJSON("EUR", "2350")
    ...
    resourceDocs += ResourceDoc(
      ...
      s"""$transactionRequestGeneralText
         ...
         |
       """.stripMargin,
      Extraction.decompose(TransactionRequestDetailsSEPAJSON(lowAmount, IbanJson("BA12 1234 5123 4513 7599 6969 977"), "This is a SEPA Transaction Request", sharedChargePolicy.toString)
      ),
      emptyObjectJson,
      emptyObjectJson :: Nil,
      true, true, true,
      ...
      case "management" :: "consumers" :: consumerId :: Nil JsonGet _ => {
        user =>
          for {
            ...
            // Format the data as json
            val json = ConsumerJSON(consumer.id, consumer.name, consumer.appType.toString(), consumer.description, consumer.developerEmail, consumer.secret, consumer.key, consumer.createdAt, consumer.updatedAt, consumer.userAuthenticationURL, consumer.isActive)
            // Return
            successJsonResponse(Extraction.decompose(json))
          }
      }
        ...
        user => {
          ...
          // Get product from the active provider
            u <- if (getProductsIsPublic)
              ...
              user ?~! ErrorMessages.UserNotLoggedIn
            bank <- Bank(bankId) ?~! {ErrorMessages.BankNotFound}
            product <- LocalMappedConnector.getProduct(bankId, productCode)?~! {ErrorMessages.ProductNotFoundByProductCode}
          } yield {
            // Format the data as json
