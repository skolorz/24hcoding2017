...
case class IbanId(val value : String) {
  override def toString = value
}

object IbanId {
  def unapply(iban : String) = Some(IbanId(iban))
}
...
case class BankAccountIbanUID(bankId : BankId, accountId : AccountId, iban : IbanId)
...
trait BankAccount {
  ...
  def setIban(iban: String): String
