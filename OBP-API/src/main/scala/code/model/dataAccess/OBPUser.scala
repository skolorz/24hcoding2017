...
import code.bankconnectors.{Connector, KafkaMappedConnector}
...
object OBPUser extends OBPUser with MetaMegaProtoUser[OBPUser]{
  ...
  def getCurrentUserId: String = {
    if(OAuthHandshake.getUser.getOrElse(None) != None)
      return OAuthHandshake.getUser.get.userId
    if(DirectLogin.getUser.getOrElse(None) != None)
      return DirectLogin.getUser.get.userId
    return ""
  }
  ...
  override def lostPassword = {
    if(Props.getBool("disable_lost_password", false)) {
      NodeSeq.Empty
    } else {
  ...
  private def testBank(bank : String, user :OBPUser) : Boolean = {

    def data = MappedAccountHolder.find(
      By(MappedAccountHolder.user, user.id),
      By(MappedAccountHolder.accountBankPermalink, bank)
    )
    !data.isEmpty
  }
  ...
  override def login = {
    var bankId = S.param("bank").openOr("")
    if (S.post_?) {
      S.param("username").
        ...
        case Full(user) if user.validated_? &&
          ...
          info("login page: " + loginPageURL)

          var redir = ""
          var developerLogin = false
          var bankOk = testBank(bankId, user)
          ...
          if(developerLogin) {
            //sprawdzamy bank
            if(!MappedAccountHolder.find(By(MappedAccountHolder.user, user.id)).isEmpty) {
              //no jeżeli jest jakiś bank to tak trochę fail
            ...
            if (bankOk) {
              shouldLogin = true
              val token = for {
                tokenParam <- S.param("_rt") ?~! "There is no Token."
                ...
                Connector.connector.vend.createLoginToken(user.id.get, token.consumerId.get).get
              }
//              redir = bankId match {
//                case "gringotts" => s"/oauth_gringotts?oauth_token=${token.openOrThrowException ("Invalid token operation")}"
//                case "gotham" => s"/oauth_gotham?oauth_token=${token.openOrThrowException ("Invalid token operation")}"
//                case _ => s"/oauth_ing?oauth_token=${token.openOrThrowException ("Invalid token operation")}"
//              }
              token match {
                case Full(properToken) => redir = s"$redir&token_id=${properToken.id}"
              }
            } else {
      ...
      performTokenRedirection(bankId)
    }
  ...
  private def performTokenRedirection(bankTemplate : String) = {
    var token = S.param("_rt")
    if(!S.param("_rt").isEmpty) {
      val redirUrl = bankTemplate match {
        case "gringotts" => s"/oauth_gringotts?oauth_token=${token.openOrThrowException("Invalid token operation")}"
        case "gotham" => s"/oauth_gotham?oauth_token=${token.openOrThrowException("Invalid token operation")}"
        case _ => s"/oauth_ing?oauth_token=${token.openOrThrowException("Invalid token operation")}"
      }

      S.redirectTo(redirUrl)
    }
  }
  ...
  override def signup = {
    if(Props.getBool("disable_dev_signup", false)) {
      NodeSeq.Empty
    } else {
