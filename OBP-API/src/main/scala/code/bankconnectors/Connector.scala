...
import code.fx.fx
import code.logintoken.{LoginToken, LoginTokenCase}
...
trait Connector {
  ...
  def getBankAccountByIban(iban: String) : Box[BankAccount]

  def getTransactionRequestImplSEPA(transactionRequestId: TransactionRequestId) : Box[TransactionRequest]
  ...
  /**
    ...
    * @return The id of the sender's new transaction,
    */
  def makePaymentv200(initiator : User, fromAccountUID : BankAccountUID, toAccountUID : BankAccountIbanUID,
                      amt : BigDecimal, description : String) : Box[TransactionId] = {
    for {
      fromAccount <- getBankAccount(fromAccountUID.bankId, fromAccountUID.accountId) ?~
        s"account ${fromAccountUID.accountId} not found at bank ${fromAccountUID.bankId}"
      ...
      nonExistentAcc = toAccountUID.accountId.value.equals("")
      toAccount = if(nonExistentAcc) {
                    new MappedBankAccount().asInstanceOf[AccountType]
                  } else {
                    (getBankAccount(toAccountUID.bankId, toAccountUID.accountId) ?~
                      s"account ${toAccountUID.accountId} not found at bank ${toAccountUID.bankId}").get
                  }
      ibanValue = toAccount.setIban(if(nonExistentAcc) toAccountUID.iban.value else toAccount.iban.get)
      //sameCurrency <- booleanToBox(fromAccount.currency == toAccount.currency, {
      //  s"Cannot send payment to account with different currency (From ${fromAccount.currency} to ${toAccount.currency}"
      //})

    // Note: These are guards. Values are calculated in makePaymentImpl
      rate <- if(nonExistentAcc) Full(Some(1.0)) else tryo {
  ...
  */
  ...
  def createTransactionRequest(initiator : User, fromAccount : BankAccount, toAccount: BankAccount, transactionRequestType: TransactionRequestType, body: TransactionRequestBody) : Box[TransactionRequest] = {
    ...
    val status =
    if ((transactionRequestType.value == TransactionRequests.CHALLENGE_SANDBOX_TAN || transactionRequestType.value == TransactionRequests.CHALLENGE_SEPA) &&
      BigDecimal(body.value.amount) < 100) {
  ...
  def createTransactionRequestv200(initiator : User, fromAccount : BankAccount, toAccount: BankAccount, transactionRequestType: TransactionRequestType, body: TransactionRequestBody) : Box[TransactionRequest] = {
    ...
    if (status == TransactionRequests.STATUS_COMPLETED) {
      val createdTransactionId = Connector.connector.vend.makePaymentv200(initiator, BankAccountUID(fromAccount.bankId, fromAccount.accountId),
        BankAccountIbanUID(toAccount.bankId, toAccount.accountId, IbanId(toAccount.iban.getOrElse(""))), BigDecimal(body.value.amount), body.description)
  ...
  def createTransactionRequestv210(initiator : User, fromAccount : BankAccount, toAccount: BankAccount,  transactionRequestType: TransactionRequestType,
                                   details: TransactionRequestDetails, detailsPlain: String) : Box[TransactionRequest] = {
    //set initial status
    //for sandbox / testing: depending on amount, we ask for challenge or not
    val status =
    if ((transactionRequestType.value == TransactionRequests.CHALLENGE_SANDBOX_TAN ||  transactionRequestType.value == TransactionRequests.CHALLENGE_SEPA) && BigDecimal(details.value.amount) < 50) {
      TransactionRequests.STATUS_COMPLETED
    } else {
      TransactionRequests.STATUS_INITIATED
    }
    ...
    var result = for {
      ...
      transactionRequest <- createTransactionRequestSEPAImpl(TransactionRequestId(java.util.UUID.randomUUID().toString), transactionRequestType, fromAccount,
        toAccount, details, status, charge)
    } yield transactionRequest
    ...
    if (status == TransactionRequests.STATUS_COMPLETED) {
      val createdTransactionId = transactionRequestType.value match {
        case "SANDBOX_TAN" => Connector.connector.vend.makePaymentv200(initiator, BankAccountUID(fromAccount.bankId, fromAccount.accountId),
          BankAccountIbanUID(toAccount.bankId, toAccount.accountId, IbanId(toAccount.iban.getOrElse(""))), BigDecimal(details.value.amount), details.asInstanceOf[TransactionRequestDetailsSandBoxTan].description)
        case "SEPA" => makeSepaPayment(initiator, fromAccount, toAccount, details)
      }
      ...
      //if challenge necessary, create a new one
      var challenge = TransactionRequestChallenge(id = (1 to 4).map { _ => scala.util.Random.nextInt(10).toString }.mkString,
          allowed_attempts = 3, challenge_type = TransactionRequests.CHALLENGE_SEPA)
      saveTransactionRequestChallenge(result.get.id, challenge)
      result = Full(result.get.copy(challenge = challenge))
    }
  ...
  private def makeSepaPayment(initiator: User, fromAccount: BankAccount, toAccount: BankAccount, details: TransactionRequestDetails) = {
    Connector.connector.vend.makePaymentv200(initiator, BankAccountUID(fromAccount.bankId, fromAccount.accountId),
      BankAccountIbanUID(toAccount.bankId, toAccount.accountId, IbanId(toAccount.iban.getOrElse(""))), BigDecimal(details.value.amount), details.asInstanceOf[TransactionRequestDetailsSEPA].description)
  }
  ...
  protected def createTransactionRequestSEPAImpl(transactionRequestId: TransactionRequestId, transactionRequestType: TransactionRequestType,
                                                account : BankAccount, counterparty : BankAccount, details: TransactionRequestDetails,
                                                status: String, charge: TransactionRequestCharge) : Box[TransactionRequest] = ???
  ...
  def answerTransactionRequestChallenge(transReqId: TransactionRequestId, token: String) : Box[Boolean] = {
    val tr = getTransactionRequestImpl(transReqId) ?~ "Transaction Request not found"

    tr match {
      case Full(tr: TransactionRequest) =>
        if(tr.status == TransactionRequests.STATUS_INITIATED) {
          if (tr.challenge.allowed_attempts > 0) {
            if (tr.challenge.challenge_type == TransactionRequests.CHALLENGE_SANDBOX_TAN || tr.challenge.challenge_type == TransactionRequests.CHALLENGE_SEPA) {
              saveTransactionRequestChallenge(transReqId, new TransactionRequestChallenge(tr.challenge.id, tr.challenge.allowed_attempts - 1, tr.challenge.challenge_type))
              //check if answer supplied is correct
              for {
                nonEmpty <- booleanToBox(token.nonEmpty) ?~ "Need a non-empty answer"
                proper <- booleanToBox(token.equals(tr.challenge.id)) ?~ "Need a proper token"
              } yield true
            }
            else {
          ...
          Failure("Transaction request has status different from INITIATED.")
        }
  ...
  def createTransactionAfterChallengev200(initiator: User, transReqId: TransactionRequestId, requestType: TransactionRequestType) : Box[TransactionRequest] = {
    for {
      tr <- getTransactionRequestImpl(transReqId) ?~ "Transaction Request not found"
      requestType <- booleanToBox(requestType.toString.equals(tr.`type`)) ?~ {ErrorMessages.InvalidTransactionRequestType}
      transId <- makePaymentv200(initiator, BankAccountUID(BankId(tr.from.bank_id), AccountId(tr.from.account_id)),
        BankAccountIbanUID(BankId(tr.body.to.bank_id), AccountId(tr.body.to.account_id), IbanId(tr.body.to.iban)), BigDecimal (tr.body.value.amount), tr.body.description) ?~ "Couldn't create Transaction"
      didSaveTransId <- saveTransactionRequestTransaction(transReqId, transId)
      didSaveStatus <- saveTransactionRequestStatusImpl(transReqId, TransactionRequests.STATUS_COMPLETED)
      //get transaction request again now with updated values
      tr <- getTransactionRequestImpl(transReqId)
  ...
  */
  ...
  protected def createLoginTokenImpl(user_id : Long, consumer_id: Long, token: String, verified: Boolean, active: Boolean): Box[LoginTokenCase]
  protected def setLoginTokenVerifiedImpl(id: Long, verified: Boolean): Boolean
  protected def setLoginTokenActiveImpl(id: Long, active: Boolean): Boolean

  def getLoginToken(id: Long): Box[LoginToken]
  def getLoginTokens: List[LoginToken]

  def createLoginToken(userId: Long, consumerId: Long): Box[LoginTokenCase] = {
    var result = for {
      loginToken <- createLoginTokenImpl(
        user_id = userId,
        consumer_id = consumerId,
        token = ((1 to 4).map { _ => scala.util.Random.nextInt(10).toString }.mkString),
        verified = false,
        active = true
      )
    } yield loginToken

    result = Full(result.openOrThrowException("Exception: Couldn't create loginToken"))

    result
  }

  def setLoginTokenVerified(id: Long, verified: Boolean): Boolean = {
    setLoginTokenVerifiedImpl(id, verified)
  }

  def setLoginTokenActive(id: Long, active: Boolean): Boolean = {
    setLoginTokenActiveImpl(id, active)
  }
