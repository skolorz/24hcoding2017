...
import code.fx.fx
import code.logintoken.{LoginTokenCase, LoginToken}
...
object LocalMappedConnector extends Connector with Loggable {
  ...
  override def getBankAccountByIban(iban: String): Box[MappedBankAccount] = {
    MappedBankAccount.find(
      By(MappedBankAccount.accountIban, iban))
  }

  override def getTransactionRequestImplSEPA(transactionRequestId: TransactionRequestId): Box[TransactionRequest] = {
  ...
  override def makePaymentImpl(fromAccount: MappedBankAccount, toAccount: MappedBankAccount, amt: BigDecimal, description: String): Box[TransactionId] = {
    ...
    val nonExistentAcc = toAccount.accountId.value.equals("")
    val rate = if(nonExistentAcc) Full(Some(1.0)) else tryo {
    ...
    val fromTransAmt = -amt
    //from account balance should decrease
  ...
  private def saveTransaction(account: MappedBankAccount, counterparty: BankAccount, amt: BigDecimal, description: String): Box[TransactionId] = {
    ...
    val mappedTransaction = if(counterparty.accountId.value.equals("")) {
      MappedTransaction.create
        ...
        .counterpartyIban(counterparty.iban.getOrElse("")).saveMe
    } else {
      MappedTransaction.create
        .bank(account.bankId.value)
        .account(account.accountId.value)
  ...
  */
  ...
  override def createTransactionRequestSEPAImpl(transactionRequestId: TransactionRequestId, transactionRequestType: TransactionRequestType,
    ...
    val mappedTransactionRequest = MappedTransactionRequest.create
      ...
      .mBody_To_BankId( counterparty.bankId.value)
      .mBody_To_AccountId( counterparty.accountId.value)
      .mBody_To_Iban( counterparty.iban.getOrElse(null))
      .mBody_Value_Currency(details.value.currency)
      .mBody_Value_Amount(details.value.amount)
      .mBody_Description(details.description)
      .mStatus(status)
      .mStartDate(now)
      .mCharge_Summary(charge.summary)
      .mCharge_Amount(charge.value.amount)
  ...
  override def createLoginTokenImpl(user_id: Long, consumer_id: Long, token: String, verified: Boolean, active: Boolean): Box[LoginTokenCase] = {
    val mappedLoginToken = LoginToken.create
      .user_id(user_id)
      ...
      .createdAt(now)
      .saveMe
    Full(mappedLoginToken).flatMap(_.toLoginToken)
  }

  override def getLoginToken(id: Long): Box[LoginToken] = {
    LoginToken.find(By(LoginToken.id, id))
  }

  override def getLoginTokens: List[LoginToken] =
    LoginToken.findAll

  override def setLoginTokenVerifiedImpl(id: Long, verified: Boolean): Boolean = {
    val result = for {
      loginToken <- getLoginToken(id)
    } yield loginToken.verified(verified).save

    result.getOrElse(false)
  }

  override def setLoginTokenActiveImpl(id: Long, active: Boolean): Boolean = {
    val result = for {
      loginToken <- getLoginToken(id)
    } yield loginToken.active(active).save
