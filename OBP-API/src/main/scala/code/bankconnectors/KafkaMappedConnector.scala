...
object KafkaMappedConnector extends Connector with CreateViewImpls with Loggable {
  ...
  override def getBankAccountByIban(iban: String): Box[MappedBankAccount] = {
    MappedBankAccount.find(
      By(MappedBankAccount.theAccountId, iban))
  }
  ...
  */
  ...
  case class KafkaBankAccount(r: KafkaInboundAccount) extends BankAccount {
    ...
    override def setIban(iban: String): String = ???
  }
  ...
  override protected def setLoginTokenVerifiedImpl(id: Long, verified: Boolean): Boolean = ???

  override def getLoginToken(id: Long): Box[LoginToken] = ???

  override def getLoginTokens: List[LoginToken] = ???

  override protected def createLoginTokenImpl(user_id: Long, consumer_id: Long, token: String, verified: Boolean, active: Boolean): Box[LoginTokenCase] = ???

  override protected def setLoginTokenActiveImpl(id: Long, active: Boolean): Boolean = ???
}
